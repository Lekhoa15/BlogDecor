<!-- resources/views/payments/card.blade.php -->

<x-guest-layout>
    <div class="container mx-auto mt-10">
        <h1 class="text-3xl">Pay with Card for Plan: {{ $plan }}</h1>
        <!-- Payment form or instructions for card payment -->
    </div>
                ...
                <div class="w-full p-10 m-4 leading-loose border border-gray-200 shadow-lg bg-gray-50">
                    ...
                    <form id="payment-form" class="space-y-8" action="{{ route('payments.store') }}" method="POST">
                        ...
                        <div class="space-y-2">
                            ...
                            <x-jet-input id="email" name="email" class="block w-full mt-1" type="text"
                                :value="auth()->user()->emailAddress() ?? old('email')" autocomplete="email" disabled />
                        ...
                        <div class="space-y-2">
                            ...
                            <x-jet-input id="city" class="block w-full mt-1" type="text" name="city"
                                :value="auth()->user()->city() ?? old('city')" autocomplete="city" required />
                        </div>

                        {{-- State --}}
                        <div class="space-y-2">
                            <x-jet-label for="state" value="{{ __('State') }}" />
                            <x-jet-input id="state" class="block w-full mt-1" type="text" name="state"
                                :value="auth()->user()->state() ?? old('state')" autocomplete="state" required />
                        </div>
                        ...
                        <div class="inline-block w-1/2 pl-2 -mx-1">
                            ...
                            <x-jet-input id="postal_code" class="block w-full mt-1" type="text" name="postal_code"
                                :value="auth()->user()->postalCode() ?? old('postal_code')" autocomplete="postal_code" required />
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <x-jet-input id="card-holder-name" class="block w-full mt-1" type="text" name="card-name"
                                :value="old('card-name')" autocomplete="card-name" />
                        </div>
