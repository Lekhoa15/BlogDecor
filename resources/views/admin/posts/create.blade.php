            ...
            <x-form action="{{ route('admin.posts.store') }}" has-files>
                <div class="space-y-8">
                    ...
                    <div>
                        ...
                        <input name="image" id="image" class="block w-full mt-1" type="file" onchange="previewImage(this)" required />
                        <x-form.error for="image" />
                    </div>

                    <!-- Script để hiển thị ảnh -->
                    <script>
                        function previewImage(input) {
                            ...
                            reader.onload = function(e) {
                                // Tìm phần tử cha của input
                                var parentDiv = input.parentNode;

                                // Tạo một phần tử hình ảnh mới
                                var img = document.createElement('img');
                                img.src = e.target.result;
                                img.style.maxWidth = '100%'; // Thiết lập kích thước tối đa cho ảnh
                                img.style.maxHeight = '200px'; // Thiết lập chiều cao tối đa cho ảnh
                                img.style.display = 'block'; // Hiển thị ảnh

                                // Xóa bất kỳ hình ảnh trước đó nếu có
                                var previousImage = parentDiv.querySelector('img');
                                if (previousImage) {
                                    parentDiv.removeChild(previousImage);
                                }

                                // Thêm ảnh vào phần tử cha
